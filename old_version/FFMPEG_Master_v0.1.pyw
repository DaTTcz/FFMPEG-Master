...
try:
    myappid = 'moje.ffmpeg.master.v0.1'
...
class SplashScreen(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        if not logo_loaded:
            self.logo_label = ctk.CTkLabel(self, text="FFMPEG Master v0.1", font=("Arial", 32, "bold"), text_color="#1f538d")
            self.logo_label.pack(pady=(60, 20))

        ctk.CTkLabel(self, text="FFMPEG Master v0.1", font=("Arial", 16, "bold"), text_color="#28a745").pack(pady=(5, 0))
...
class App(ctk.CTk, TkinterDnD.DnDWrapper):
    def __init__(self):
        ...
        # --- UI ---
        ctk.CTkLabel(self, text="FFMPEG Master v0.1", font=("Arial", 22, "bold")).pack(pady=10)
        ...
        self.drop_label = ctk.CTkLabel(self.drop_frame, text="Sem přetáhni MKV soubory")
    ...
    def browse_files(self):
        files = filedialog.askopenfilenames(filetypes=[("MKV soubory", "*.mkv")])
        for f in files:
            ...
            if self.current_process.returncode == 0 and not self.stop_requested:
                ...
                try:
                    ...
                    if src_drive == dst_drive:
                        ...
                        new_name_path = os.path.join(os.path.dirname(file_path), os.path.splitext(filename)[0] + "_hotovo.mkv")
